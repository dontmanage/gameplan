...
def make_unsplash_request(path):
	if not "unsplash_access_key" in dontmanage.conf:
		dontmanage.throw("Please set unsplash_access_key in site_config.json")
	...
	res = requests.get(
		...
		headers={
			"Accept-Version": "v1",
			"Authorization": f"Client-ID {dontmanage.conf.unsplash_access_key}",
		},
	)
