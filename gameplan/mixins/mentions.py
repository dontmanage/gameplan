		...
		for mention in mentions:
			...
			if self.doctype == "GP Task":
				...
				if self.reference_doctype == "GP Discussion":
					...
					values.project = dontmanage.db.get_value("GP Task", self.reference_name, "project")

			if dontmanage.db.exists("GP Notification", values):
				continue
			notification = dontmanage.get_doc(doctype='GP Notification')
