...
def migrate_posts():
	...
	for i, topic in enumerate(topics):
		if dontmanage.db.exists('Discourse ID Map', {'discourse_table': 'topics', 'discourse_id': topic.id}):
			continue
		...
		try:
			...
			doc = dontmanage.get_doc(
				...
				project=project,
				team=dontmanage.db.get_value('GP Project', project, 'team'),
			...
			for comment in comments:
				...
				comment_doc = dontmanage.get_doc(
					doctype='GP Comment',
			...
			for d in visits_data:
				name = dontmanage.db.get_next_sequence_val('GP Discussion Visit')
...
def save_image(src, filename=None, doc=None):
	if not src.startswith('http'):
		src = urljoin('https://gameplan.dontmanage.io', src)
	...
	if res.ok:
		_, extn = splitext(src)
		hash = dontmanage.generate_hash(length=5)
...
def get_user(user_id):
	return dontmanage.db.get_value('Discourse ID Map', {
...
def get_avatar_url(user_id):
	...
	if user:
		return f'https://gameplan.dontmanage.io/user_avatar/gameplan.dontmanage.io/{user.username}/240/{user.custom_upload_id}.png'


def clear_data(doctypes=None):
	to_delete = []
	for dt in doctypes:
		for row in dontmanage.db.get_all('Discourse ID Map', {'reference_doctype': dt}, ['reference_name', 'name']):
...
def migrate_categories():
	...
	for i, d in enumerate(categories):
		...
		if not dontmanage.db.exists('GP Team', {'title': team}):
			team_doc = dontmanage.get_doc(doctype='GP Team', title=team).insert()
			log_discourse_map(team_doc, 'categories', id)
		else:
			team_doc = dontmanage.get_doc('GP Team', {'title': team})

		project_doc = dontmanage.get_doc(doctype='GP Project', title=project, team=team_doc.name).insert()
...
def run_query(query, values=None):
	...
	return [dontmanage._dict(row) for row in result]
