...
class GPTask(HasMentions, Document):
	...
	def update_tasks_count(self, delta=1):
		current_tasks_count = dontmanage.db.get_value("GP Project", self.project, "tasks_count")
		dontmanage.db.set_value("GP Project", self.project, "tasks_count", current_tasks_count + delta)

	def update_project_progress(self):
		if self.project and self.has_value_changed("is_completed"):
			dontmanage.get_doc("GP Project", self.project).update_progress()
