...
from dontmanage.utils import strip_html_tags, update_progress_bar
...
def search(query, start=0):
	...
	for d in result.docs:
		...
		names.append(dontmanage.utils.cint(name))
	names = list(set(names))

	data_by_name = {
		d.name: d
		for d in dontmanage.db.get_all('GP Discussion',
	...
	for d in result.docs:
		_, name, comment = d.id.split(':')
		name = dontmanage.utils.cint(name)
		if name in seen:
			continue
...
def rebuild_index_in_background():
	if not dontmanage.cache().get_value('discussions_index_in_progress'):
		dontmanage.enqueue(rebuild_index, queue='long')


def rebuild_index_if_not_exists():
	try:
		dontmanage.cache().ft(INDEX_NAME).info()
	except ResponseError:
		rebuild_index()
...
def create_index_for_records(records):
	...
	for i, d in enumerate(records):
		if not hasattr(dontmanage.local, 'request') and len(records) > 10:
		...
		mapping = {
			...
			"modified": dontmanage.utils.cstr(d.modified),
			"comment": d.comment,
		}
...
def get_records_to_index():
	records = []
	result = dontmanage.db.get_all('GP Discussion', fields=['name', 'title', 'content', 'modified'])
	for d in result:
		d.comment = ''
		records.append(d)

	result = dontmanage.db.get_all('GP Comment',
