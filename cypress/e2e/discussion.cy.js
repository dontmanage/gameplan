describe('Discussion', () => {
  it('all discussion actions', () => {
    ...
    cy.request('POST', '/api/method/dontmanage.client.insert_many', {
      docs: [
        {
          doctype: 'GP Team',
          title: 'Engineering',
    ...
    cy.intercept({
      method: 'POST',
      url: '/api/method/dontmanage.client.insert',
      times: 1,
    }).as('discussion')
    ...
    // add comment
    cy.intercept('POST', '/api/method/dontmanage.client.insert').as('comment')
    ...
    cy.get('li:contains("DontManageErp")').click()
    cy.button('Move to DontManageErp').click()

    cy.get('@data').then((data) => {
      let dontmanageerpProject = data[2]
      cy.get('@discussion')
        .its('response.body.message')
        .then((discussion) => {
          cy.url().should(
            'include',
            `/g/engineering/projects/${dontmanageerpProject}/discussion/${discussion.name}`
          )
        })
    })
